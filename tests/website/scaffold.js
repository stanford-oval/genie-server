...
function request(url, method, data, options = {}) {
    options['user-agent'] = 'Almond-Test/1.0.0';
    options.debug = DEBUG;

    if (method === 'POST' && (typeof data !== 'string' && data !== null && !(data instanceof Buffer)))
        return Tp.Helpers.Http.postStream(SERVER_ORIGIN + url, data, options);
    else
        return Tp.Helpers.Http.request(SERVER_ORIGIN + url, method, data, options);
}
...
function assertRedirect(request, redirect) {
    return request.then(() => {
        ...
        if (typeof redirect === 'function')
            ...
            assert.strictEqual(err.redirect, Url.resolve(SERVER_ORIGIN, redirect));
    });
}
